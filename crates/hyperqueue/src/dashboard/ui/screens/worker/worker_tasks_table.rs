...
pub struct WorkerTasksTable {
    table: StatefulTable<WorkerTasksRow>,
}
...
impl WorkerTasksTable {
    ...
    pub fn draw(&mut self, rect: Rect, frame: &mut DashboardFrame) {
        //todo: get data from JobInfo
        self.table.draw(
            rect,
            frame,
            TableColumnHeaders {
                title: "task history for worker".to_string(),
...
fn create_rows(overview: Vec<&WorkerOverview>) -> Vec<WorkerTasksRow> {
    overview
        .iter()
        .map(|worker| {
