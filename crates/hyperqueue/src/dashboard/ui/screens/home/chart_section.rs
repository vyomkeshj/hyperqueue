...
pub enum TimeEventType {
    ...
    /// The average cpu utilization of the worker.
    AvgCpuUsageUpdate(f32),
}
...
impl ClusterOverviewChart {
    pub fn update(&mut self, overview: CollectedOverview) {
        ...
        let interpolation =
            interpolate_y_axis(self.current_conn_workers as f64, num_workers as f64, 30_f64);
    ...
    pub fn draw(&mut self, rect: Rect, frame: &mut DashboardFrame) {
        let worker_counts: Vec<(f64, f64)> = self
            ...
            .iter()
            .filter_map(|event| {
                if let TimeEventType::WorkerCountUpdate(count) = event.clone().event {
                    //fixme: how bad?
